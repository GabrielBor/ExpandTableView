...
- (void) initialize {
    ...
    self.separatorStyle = UITableViewCellSeparatorStyleNone; // Mexi aqui, estado da separação de células
...
//            NSLog(@"reusing existing JKParentTableViewCell");
        ...
        if ([self.tableViewDelegate respondsToSelector:@selector(fontForParents)]) {
            ...
            [cell.labelFather setFont:font];
        }

        //Seta aqui os textos das labels
        NSString * labelStr = [self.dataSourceDelegate labelForParentCellAtIndex: parentIndex];
        [[cell labelFather] setText:labelStr];
        
        NSString * labelStrTotalFriends = [self.dataSourceDelegate labelTotalFriendsCellAtIndex: parentIndex];
        [[cell labelTotalFriends] setText:labelStrTotalFriends];
        
        NSString * labelStrYear = [self.dataSourceDelegate labeYearCellAtIndex:parentIndex];
        [[cell labelYear] setText:labelStrYear];
        
        NSString * labelStrQtFriends = [self.dataSourceDelegate labeQtFriendsCellAtIndex:parentIndex];
        [[cell labelQtFriends] setText:labelStrQtFriends];
...
- (NSString *) labelDateAndHourIndex:(NSInteger)childIndex underParentIndex:(NSInteger)parentIndex {
    return [self.dataSourceDelegate labelDateAndHourChildIndex:childIndex withinParentCellIndex:parentIndex];
}
